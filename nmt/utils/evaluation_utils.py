...
def evaluate(ref_file, trans_file, metric, subword_option=None):
  ...
  elif metric.lower() == "amino_acid_accuracy":
    evaluation_score = _amino_acid_accuracy(ref_file, trans_file)
  else:
  ...
  with codecs.getreader("utf-8")(tf.gfile.GFile(label_file, "rb")) as label_fh:
    with codecs.getreader("utf-8")(tf.gfile.GFile(pred_file, "rb")) as pred_fh:
      ...
      for label in label_fh:
        label = label.strip()
        label = label.replace('L', 'I') ##### <<<<< change L in true label to I
...
def _amino_acid_accuracy(label_file, pred_file):
  """Compute amino acid accuracy."""
  ...
  with codecs.getreader("utf-8")(tf.gfile.GFile(label_file, "rb")) as label_fh:
    with codecs.getreader("utf-8")(tf.gfile.GFile(pred_file, "rb")) as pred_fh:
      ...
      for sentence in label_fh:
        labels = sentence.strip().split(" ")
        labels = [input_config.mass_AA[aa] for aa in labels]
        # print(labels)
        preds = pred_fh.readline().strip().split(" ")
        if not preds[0]:
          preds = ['<unk>']
        preds = [input_config.mass_AA[aa] for aa in preds]
        # print(preds)
        pred_len, label_len = len(preds), len(labels)
        pos_pred, pos_label = 0, 0
        sum_pred, sum_label = 0., 0.
        match = 0.0
        while pos_pred < pred_len and pos_label < label_len:
          if abs(sum_pred - sum_label) < 0.03 and abs(preds[pos_pred] - labels[pos_label]) < 0.0001:
            match += 1
            sum_pred += preds[pos_pred]
            sum_label += labels[pos_label]
            pos_pred += 1
            pos_label += 1
          else:
            if sum_pred < sum_label:
              sum_pred += preds[pos_pred]
              pos_pred += 1
            else:
              sum_label += labels[pos_label]
  ...
  return 100 * total_match / total_count
